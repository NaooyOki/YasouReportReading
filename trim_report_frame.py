...
def trim_paper_frame(img):
    ...
    @return :image 背景部分をトリミングして、記録用紙だけになった画像イメージ
      記録用紙をスキャナーで読み取った場合は、暗い背景はないので元の画像イメージをそのまま返す
    """
    img_debug = img.copy()
    ...
    cv2.imwrite("./tmp/trim_paper_frame_input.jpg", img_gray2)
    ...
    for i, contour in enumerate(contours):
        ...
        if ((w > img_gray2.shape[1] * 0.5) and (h > img_gray2.shape[0] * 0.5)):
            ...
            if (__debug__):
                ...
                cv2.imwrite("./tmp/trim_paper_frame_output.jpg", img_debug)
            return (trim_img)
